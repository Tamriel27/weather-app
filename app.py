...
def get_data():
    ...
    response = request.urlretrieve(url, 'data/tsag_agaar.xls')
    return response

def convert_to_xlsx():
        with open('data/tsag_agaar.xls', encoding='utf-8') as xml_file:
            soup = bs(xml_file.read(), 'xml')
            writer = pd.ExcelWriter('data/tsag_agaar.xlsx')
...
df = pd.read_excel('data/tsag_agaar.xlsx', skiprows=1)
